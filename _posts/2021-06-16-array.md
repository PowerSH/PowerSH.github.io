...
이번 포스팅에서는 `list`와 `array`에 대해서 간략히 다뤄보겠습니다.
...
```python
list_A = [1, 2, 3]
list_A.insert(3, 4)   # [1, 2, 3, 4]
print(list_A)
list_A.insert(3, 'a') # [1, 2, 3, 'a', 4]
print(list_A)
```
...
`remove()`함수는 `insert()`함수와는 다르게 특정 `index`에 있는 요소를 삭제하는 것이 아닌 특정 요소를 찾아서 삭제합니다. 위의 코드를 보면 `my_list.remove(2)`는 `2`번 `index`의 내용을 삭제하는 것이 아니라 `2`라는 요소를 삭제한 것임을 알 수 있으며 `my_list.remove('a')`로 요소를 찾아 삭제한다는 것을 알 수 있습니다.
